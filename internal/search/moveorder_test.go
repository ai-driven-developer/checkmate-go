...
func TestMoveOrderHashMoveFirst(t *testing.T) {
	...
	OrderMoves(&ml, quiet2, [2]board.Move{}, board.NullMove, nil, 0, nil)
...
func TestMoveOrderKillerMovePriority(t *testing.T) {
	...
	OrderMoves(&ml, board.NullMove, killers, board.NullMove, nil, 0, nil)
...
func TestMoveOrderCountermovePriority(t *testing.T) {
	var ml board.MoveList
	quiet1 := board.NewMove(board.E2, board.E4, board.FlagDoublePawn, board.Pawn, board.NoPiece)
	capture := board.NewMove(board.D4, board.E5, board.FlagCapture, board.Pawn, board.Queen)
	quiet2 := board.NewMove(board.G1, board.F3, board.FlagQuiet, board.Knight, board.NoPiece) // killer
	quiet3 := board.NewMove(board.B1, board.C3, board.FlagQuiet, board.Knight, board.NoPiece) // countermove
	quiet4 := board.NewMove(board.D2, board.D3, board.FlagQuiet, board.Pawn, board.NoPiece)
	...
	killers := [2]board.Move{quiet2, board.NullMove}
	OrderMoves(&ml, board.NullMove, killers, quiet3, nil, 0, nil)

	// Expected order: capture, killer (quiet2), countermove (quiet3), quiet1, quiet4.
	if ml.Moves[0] != capture {
		t.Errorf("capture should be first, got %v", ml.Moves[0])
	}
	if ml.Moves[1] != quiet2 {
		t.Errorf("killer should be second, got %v", ml.Moves[1])
	}
	if ml.Moves[2] != quiet3 {
		t.Errorf("countermove should be third, got %v", ml.Moves[2])
	}
}

func TestMoveOrderCountermoveNotDuplicateKiller(t *testing.T) {
	// When the countermove is also a killer, it should get the killer score
	// (500k) not the countermove score (400k), so it doesn't appear twice.
	var ml board.MoveList
	quiet1 := board.NewMove(board.E2, board.E4, board.FlagDoublePawn, board.Pawn, board.NoPiece)
	quiet2 := board.NewMove(board.G1, board.F3, board.FlagQuiet, board.Knight, board.NoPiece)
	...
	killers := [2]board.Move{quiet2, board.NullMove}
	// quiet2 is both killer and countermove.
	OrderMoves(&ml, board.NullMove, killers, quiet2, nil, 0, nil)

	// quiet2 should still come first (killer score 500k, not countermove 400k).
	if ml.Moves[0] != quiet2 {
		t.Errorf("killer/countermove should be first, got %v", ml.Moves[0])
	}
}
